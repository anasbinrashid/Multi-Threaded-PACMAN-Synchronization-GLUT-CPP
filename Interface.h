 ...
 * scorePrint the current score in the game
 */
...
void drawScore(int scorePrint)
{
    ...
    while(i>=0)										// Iterate through the number and draw the correct digit
    {
    	if(str[i]=='0')
    	{
 ...
 * liveCount number of lives pacman has left in current game
 */
...
void drawLives(int liveCount)
{
    ...
    while(i<liveCount-1)				    // Draw as many as icons as pacman has lives left
    {
 ...
 * score the score at the end of the current game
 */
